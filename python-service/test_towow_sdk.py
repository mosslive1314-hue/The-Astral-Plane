...
from agent_sync import AgentSyncService
...
async def test_towow_sdk():
    logger.info("Starting TongYao SDK test...")

    agent_sync = AgentSyncService()
    sync_result = await agent_sync.sync_all_agents()
    logger.info(f"Synced {sync_result['synced']} agents")
    ...
    agent_vectors = {}
    display_names = {}
    for agent_id, agent_data in agent_sync.AGENTS_DB.items():
        try:
            vector = await agent_sync._encode_agent_profile(agent_data)
