...
import { Mic, Send, Radio, User, Code, FileText, Wifi, Zap, ShoppingBag, CheckCircle, Loader2, Sparkles } from 'lucide-react'
import { toast } from 'sonner'
import { useRouter } from 'next/navigation'
import { broadcastRequirement, KStarConfig, resonanceEngine } from '@/lib/towow-api'
...
export function ResonanceEngine() {
  ...
  useEffect(() => {
    const loadAgents = async () => {
      ...
      try {
        const response = await fetch(`${TOWOW_API_URL}/api/agents`)
        if (!response.ok) {
        ...
        for (const agent of agents) {
          if (agent.id && agent.vector) {
            const hdcVector = {
              data: agent.vector,
              dimension: agent.vector.length
            }
            resonanceEngine.registerAgent(agent.id, hdcVector)
          }
        }
        
        console.log(`已注册 ${agents.length} 个 Agents 到 resonanceEngine`)
        setAgentsLoaded(true)
      } catch (error) {
        console.error('加载 Agents 失败:', error)
        toast.warning('无法加载 Agent 数据，广播功能可能受限')
      }
    }
    
    loadAgents()
  }, [agentsLoaded, TOWOW_API_URL])
  ...
  const handleBroadcast = async () => {
    ...
    try {
      const kStarConfig: KStarConfig = { k: 10 }
      const response = await broadcastRequirement(query, kStarConfig)
      ...
      const resonatingItems: ResonatingItem[] = response.agents.map((offer, index) => {
        ...
        return {
          ...
          desc: offer.offer_content 
            ? `根据您的需求，这位专家的共鸣分数为 ${(offer.resonance_score * 100).toFixed(1)}%。`
            : '等待更多详细信息...',
      ...
      toast.success('灵波已发射', {
        description: `通过通爻协议，找到 ${resonatingItems.length} 个共鸣的智能体`
      })
    } catch (error: any) {
